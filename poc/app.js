...
function updateVoiceStatus() {
  ...
  // Pre-recorded MP3 voice clips are bundled with the app, so the
  // "install a system voice" and "switch to Edge" hints no longer apply.
  // Force them hidden regardless of speechSynthesis availability.
...
document.getElementById('btn-go-setup').addEventListener('click', () => {
  Sound.init(); Sound.resume(); Sound.tap();
  setTimeout(() => {
    // First-time users get the calm onboarding flow; returning users skip straight to setup.
    if (!state.profile.onboarded) {
      gotoOnbStep('welcome');
      return;
    }
...
const DebugPanel = {
  ...
  _render() {
    ...
    // Refresh SW stats opportunistically (results arrive async via postMessage)
    try { navigator.serviceWorker?.controller?.postMessage?.({ type: 'GET_STATS' }); } catch (_) {}

    // ── Voice/audio diagnostics ──
    const voiceEl = document.getElementById('debug-voice');
    if (voiceEl) {
      const d = collectVoiceDiagnostics();
      const stats = window.__SW_STATS || {};
      const total = (stats.hits || 0) + (stats.misses || 0);
      const hitPct = total > 0 ? Math.round(100 * stats.hits / total) : 0;
      const lastClip = (typeof VoiceBus !== 'undefined' && VoiceBus.lastClip) ? VoiceBus.lastClip : '—';
      const totalPlays = (typeof VoiceBus !== 'undefined') ? VoiceBus.totalPlays : 0;

      const row = (label, value, hint) => `
        <div class="live-event">
          <span class="le-name">${label}${hint ? ` <span style="opacity:.6;font-weight:normal">(${hint})</span>` : ''}</span>
          <span class="le-time" style="direction:ltr;max-width:60%;overflow:hidden;text-overflow:ellipsis;white-space:nowrap">${value}</span>
        </div>`;
      voiceEl.innerHTML = [
        row('SW version',          d.swVersion,        d.swActive ? '✓ مفعّل' : '✕ لم يُسجَّل'),
        row('Manifest version',    'v' + (d.cloud.manifestVersion || '?'), `${d.cloud.phraseCount} عبارة`),
        row('Audio MP3s in cache', stats.audioCount || 0, total > 0 ? `${stats.hits} hits · ${stats.misses} misses (${hitPct}% hit)` : 'لم تبدأ بعد'),
        row('Last clip played',    lastClip, totalPlays + ' total'),
        row('Caches',              d.cacheCount,       d.cacheNames.join(' · ') || '—'),
        row('VoiceBus',            d.bus.busy ? 'يعمل' : 'صامت', `backend=${d.bus.backend || '—'} · token=${d.bus.token}`),
        row('Cloud manifest',      d.cloud.ready ? 'محمّل' : 'غير محمّل', `voice=${d.cloud.voice || '—'}`),
        row('Active language',     d.lang,             d.langDir),
        row('Arabic OS voice',     d.arVoice || '✕ غير مثبّت', d.arVoice ? '' : 'TTS العربي صامت'),
        row('English OS voice',    d.enVoice || '✕ غير مثبّت', ''),
        row('Online',              navigator.onLine ? 'نعم' : 'لا', ''),
        `<button type="button" id="debug-hard-update" style="margin-top:12px;padding:10px 14px;background:#DC2626;color:white;border:0;border-radius:8px;font-weight:700;width:100%;cursor:pointer">⚠️ Hard Update — مسح الكاش وإعادة التحميل</button>`,
      ].join('');

      // (Re-)wire the button on every render since innerHTML wipes listeners
      voiceEl.querySelector('#debug-hard-update')?.addEventListener('click', () => {
        if (confirm('سيتم مسح كل cache + service worker + إعادة التحميل. متابعة؟')) {
          brightMindsHardUpdate();
        }
      });
    }
...
// ════════════════════════════════════════════════════════════
// VOICE / AUDIO DIAGNOSTICS — single source of truth used by both
// the Debug Panel and window.brightMindsAudit().
// Returns a frozen snapshot of every audio-related setting and runtime
// state so issues can be diagnosed on-device without DevTools access.
// ════════════════════════════════════════════════════════════
function collectVoiceDiagnostics() {
  ...
  const cloudReady = !!(typeof CloudVoice !== 'undefined' && CloudVoice.manifest);
  const phraseCount = cloudReady ? Object.keys(CloudVoice.manifest.phrases || {}).length : 0;

  // SW + cache details (best-effort; some browsers restrict these)
  const sw = navigator.serviceWorker?.controller || null;
  const reg = navigator.serviceWorker;

  return {
    swVersion: window.__SW_VERSION || '(لم يُبلَّغ)',
    swActive:  !!sw,
    cacheCount: window.__CACHE_NAMES?.length || 0,
    cacheNames: window.__CACHE_NAMES || [],
    bus: {
      busy:    typeof VoiceBus !== 'undefined' ? VoiceBus._busy    : false,
      backend: typeof VoiceBus !== 'undefined' ? VoiceBus._currentBackend : null,
      token:   typeof VoiceBus !== 'undefined' ? VoiceBus._currentToken : 0,
    },
    cloud: {
      ready: cloudReady,
      voice: cloudReady ? CloudVoice.getVoice(lang) : null,
      phraseCount,
      manifestVersion: cloudReady ? CloudVoice.manifest.version : null,
    },
    lang, langDir: lang === 'ar' ? 'rtl' : 'ltr',
    arVoice: arVoice ? `${arVoice.name} (${arVoice.lang})` : null,
    enVoice: enVoice ? `${enVoice.name} (${enVoice.lang})` : null,
    queued: window.speechSynthesis ? (window.speechSynthesis.pending ? 'مُعلَّق' : '0') : 'N/A',
  };
}

// Refresh cached cache-names list (cheap; runs once at boot and after SW update)
async function _refreshCacheNames() {
  if (!window.caches) return;
  try { window.__CACHE_NAMES = await caches.keys(); } catch (_) { window.__CACHE_NAMES = []; }
}
_refreshCacheNames();

// ── SW version + stats comms ──
// The SW posts its CACHE_VERSION on activate via postMessage. We stash it on
// window.__SW_VERSION so the Debug Panel can display it, and on receipt we
// also print a LOUD banner the user can spot on mobile DevTools.
window.__SW_STATS = { hits: 0, misses: 0, audioCount: 0 };

function _printVoiceBanner(ver) {
  try {
    console.log('%c🎧 VOICE SYSTEM ACTIVE ' + ver,
      'background:#8B5CF6;color:white;padding:6px 12px;border-radius:6px;' +
...
if ('serviceWorker' in navigator) {
  navigator.serviceWorker.addEventListener('message', (e) => {
    ...
    if (e.data.type === 'SW_VERSION') {
      const isNew = window.__SW_VERSION && window.__SW_VERSION !== e.data.version;
      window.__SW_VERSION = e.data.version;
      _printVoiceBanner(e.data.version);
      _refreshCacheNames();
      if (isNew) console.log('[SW] version changed → page reload imminent');
    } else if (e.data.type === 'SW_STATS') {
      window.__SW_STATS = {
        hits:       e.data.hits || 0,
        misses:     e.data.misses || 0,
        audioCount: e.data.audioCount || 0,
        version:    e.data.version,
      };
    }
  });
  // Ask the active SW to report its version + stats (in case we missed install)
  navigator.serviceWorker.ready.then(reg => {
    reg.active?.postMessage?.({ type: 'GET_VERSION' });
    reg.active?.postMessage?.({ type: 'GET_STATS' });
  }).catch(() => {});
  // Print a banner even before the SW responds, with the cached value if any
  setTimeout(() => { if (!window.__SW_VERSION) _printVoiceBanner('(SW not yet active)'); }, 800);
}

// ════════════════════════════════════════════════════════════
// HARD UPDATE — nuke everything and re-bootstrap from network.
// Exposed via window.brightMindsHardUpdate() and the Debug Panel button.
// Use when caches get stuck on an old version (mobile Safari especially).
// ════════════════════════════════════════════════════════════
async function brightMindsHardUpdate() {
  console.log('%c⚠️ HARD UPDATE — wiping caches + SWs', 'color:#DC2626;font-weight:bold');
  try {
    if ('serviceWorker' in navigator) {
    ...
    if (window.localStorage) {
      try { localStorage.removeItem('brightminds.analytics.v1'); } catch (_) {}
    }
  } finally {
    // Bypass HTTP cache via cache-buster query (location.reload(true) is deprecated)
    const u = new URL(window.location.href);
    u.searchParams.set('_hu', Date.now());
    window.location.replace(u.toString());
  }
}
window.brightMindsHardUpdate = brightMindsHardUpdate;

// ════════════════════════════════════════════════════════════
// AUDIT — list every i18n speak.* key that has NO bundled MP3 and
// therefore falls back to Web Speech TTS (which is silent on devices
// without an installed Arabic voice).
// Static keys can be pre-recorded; dynamic ones need template-per-value
// generation. Use this to plan future audio batches.
// ════════════════════════════════════════════════════════════
const KEYS_USED_AT_RUNTIME = [
  // From speakI18n calls in app.js
  'speak.add_count', 'speak.add_problem', 'speak.colorhunt_prompt',
  'speak.count', 'speak.draw_horizontal', 'speak.draw_vertical',
  'speak.letters_correct', 'speak.letters_prompt', 'speak.maze_win',
  'speak.next_digit', 'speak.next_exercise', 'speak.puzzle_win',
  'speak.shop_win', 'speak.story_dragon', 'speak.test_phrase',
  'speak.weak_focus', 'speak.write',
  // From speakNamed calls (each may have a _named variant too)
  'speak.add_intro', 'speak.colorhunt_intro', 'speak.findletter_intro',
  'speak.letters_intro', 'speak.maze_intro', 'speak.memory_intro',
  'speak.puzzle_intro', 'speak.science_intro', 'speak.session_end',
  'speak.shop_intro', 'speak.story_intro', 'speak.story_read_intro',
  'speak.wb_intro',
];
// Keys whose source string contains {placeholder} → can't be a single MP3
const DYNAMIC_KEYS = new Set([
  'speak.add_problem', 'speak.colorhunt_prompt', 'speak.letters_correct',
  'speak.letters_prompt', 'speak.story_dragon', 'speak.draw_vertical',
  'speak.draw_horizontal',
]);

function auditMissingClips(lang = 'ar') {
  const map = (typeof CLOUD_KEY_MAP !== 'undefined') ? CLOUD_KEY_MAP : {};
  const out = { covered: [], staticMissing: [], dynamicMissing: [] };
  for (const key of KEYS_USED_AT_RUNTIME) {
    const clip = map[key];
    const bundled = clip && typeof CloudVoice !== 'undefined' && CloudVoice.has(clip, lang);
    const where = inferGame(key);
    if (bundled) {
      out.covered.push({ key, clip, game: where });
    } else if (DYNAMIC_KEYS.has(key)) {
      out.dynamicMissing.push({ key, game: where, reason: 'يحتوي متغيّر — يحتاج توليد قالب لكل قيمة' });
    } else {
      out.staticMissing.push({ key, game: where, suggestion: 'يمكن تسجيل MP3 واحد ثابت' });
    }
  }
  return out;
}

function inferGame(key) {
  const map = {
    'add_': 'الجمع', 'colorhunt_': 'صيد الألوان', 'letters_': 'الحروف',
    'findletter_': 'ابحث عن الحرف', 'memory_': 'الذاكرة', 'maze_': 'المتاهة',
    'puzzle_': 'الأحجية', 'shop_': 'المتجر', 'story_': 'القصص',
    'wb_': 'بناء الكلمات', 'science_': 'العلوم', 'session_': 'الجلسة',
    'draw_': 'الضرب (رسم)', 'count': 'الضرب (عد)', 'write': 'الضرب (كتابة)',
    'next_': 'انتقال', 'weak_': 'تحفيز', 'test_': 'اختبار الصوت',
  };
...
window.auditMissingClips = auditMissingClips;

// Console helper for on-device audits
window.brightMindsAudit = function () {
  const d = collectVoiceDiagnostics();
  const m = auditMissingClips(d.lang);
  /* eslint-disable no-console */
  console.group('%c🎧 BrightMinds Audio Audit', 'color:#8B5CF6;font-weight:bold;font-size:14px');
  console.table(d.bus);
  console.log('SW:', d.swVersion, d.swActive ? '(active)' : '(none)');
  console.log('Caches:', d.cacheNames);
  console.log('Cloud manifest:', d.cloud);
  console.log('OS voices: ar =', d.arVoice || '—', '· en =', d.enVoice || '—');
  console.log('Language:', d.lang, '· Queued:', d.queued);
  console.log(`✅ MP3 covered: ${m.covered.length}`);
  console.log(`📝 Static missing (can pre-record): ${m.staticMissing.length}`, m.staticMissing);
  console.log(`🔀 Dynamic missing (template-per-value): ${m.dynamicMissing.length}`, m.dynamicMissing);
  console.groupEnd();
  /* eslint-enable no-console */
  return { ...d, missingClips: m };
};
...
function initColorHuntLesson() {
  ...
  setTimeout(() => speakI18n('speak.colorhunt_prompt', {
    color:   state.colorProblem.targetName,
    colorId: state.colorProblem.target.id,
  }), state.session.current === 0 ? 2400 : 100);
}
...
// Narrate the current story page. Uses pre-recorded Salma/Aria MP3 clips
// (key pattern: s_<storyId>_p<idx>  or  s_<storyId>_m for the moral) so the
// kid always hears the same warm teacher voice. If the clip is missing for any
// reason, we stay silent for Arabic (avoids OS TTS mispronouncing) but allow
// English TTS as a fallback (since English voices are usually high-quality).
function _storyClipKey(sp) {
  if (!sp) return null;
  if (sp.showingMoral) return 's_' + sp.story.id + '_m';
  return 's_' + sp.story.id + '_p' + sp.pageIdx;
}
...
function narrateStoryPage() {
  ...
  const key = _storyClipKey(sp);

  if (key && typeof CloudVoice !== 'undefined' && CloudVoice.has(key, lang)) {
    CloudVoice.play(key, lang, { replace: true });
    _preloadNextStoryClip(sp, lang);
    return;
  }
  // No clip available. For Arabic we refuse to fall back to OS TTS — silence
  // is better than wrong pronunciation. English TTS is reliable, so allow it.
  if (lang === 'ar') return;
  if (typeof Voice === 'undefined') return;
  const text = sp.showingMoral ? sp.story.moral : sp.story.pages[sp.pageIdx].text;
  Voice.speak(text, lang, { rate: 0.95, replace: true });
}

// Warm the HTTP cache for the next page's clip so playback is gapless when the
// kid taps "next". The current clip is already streaming; this is best-effort.
function _preloadNextStoryClip(sp, lang) {
  if (typeof CloudVoice === 'undefined') return;
  const total = sp.story.pages.length;
  const candidates = [];
  if (!sp.showingMoral && sp.pageIdx + 1 < total) {
    candidates.push('s_' + sp.story.id + '_p' + (sp.pageIdx + 1));
  }
  if (!sp.showingMoral && sp.pageIdx === total - 1) {
    candidates.push('s_' + sp.story.id + '_m');
  }
  for (const k of candidates) {
    if (!CloudVoice.has(k, lang)) continue;
    try {
      const link = document.createElement('link');
      link.rel = 'preload';
      link.as = 'audio';
      link.href = CloudVoice.url(k, lang);
      link.id = '_audio_preload_' + k;
      // Avoid stacking dozens of identical preload tags
      document.getElementById(link.id)?.remove();
      document.head.appendChild(link);
      // Strip after a few seconds so it doesn't hang around forever
...
// ════════════════════════════════════════════════════════════
// ── Service Worker registration + automatic hard-refresh ──
// The SW uses skipWaiting() + clients.claim() so a new version takes over
// immediately. When that happens the `controllerchange` event fires on the
// page — we reload once so the kid sees the latest assets and code without
// having to manually clear cache. Guarded by a flag so we don't reload-loop.
...
if ('serviceWorker' in navigator) {
  ...
  navigator.serviceWorker.addEventListener('controllerchange', () => {
    ...
    _swReloadGuard = true;
    try { console.log('[SW] new version active — reloading'); } catch (_) {}
...
function finishSplash() {
  ...
  if (typeof Mascot !== 'undefined') Mascot.mount('mascot-container');
  // Always show language picker first — returning users can quickly proceed,
  // first-time users get a clear language + voice choice before onboarding.
  showScreen('language');
}
