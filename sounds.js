...
const Voice = {
  ...
  speak(text, lang = 'ar', opts = {}) {
    ...
    // `opts.replace: true` cancels any pending/playing utterance first so this
    // one plays immediately. Use for reactive sounds (count) to avoid backlog.
    if (opts.replace && window.speechSynthesis) {
